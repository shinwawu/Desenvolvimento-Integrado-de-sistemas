...
def CGNR_function(matrix_sparse: sp.csc_matrix, g: np.ndarray, tol: float = 1e-5, max_iter: int = 10):
...
def run_cgnr(model_id: str, g: np.ndarray):
    ...
    print(f"modelo {model_id}: convergiu em {iteracoes} iteracoes com erro final {erro_final:.6e}")
...
async def reconstruct(model_id: str, sinal: Sinal):
    ...
    return {"message": f"Reconstrução para o modelo {model_id} finalizada.", "image": img}
